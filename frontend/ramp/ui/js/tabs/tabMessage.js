...
export function renderMessageTable() {
    ...
    const targetDate = document.getElementById('msgTargetDate').value;

    const allRF = itemsDataSet.get({
        filter: i => {
            if (i.isOther || i.isActive === false || i.isLink || !i.flight.startsWith('RF')) return false;
    ...
    allRF.sort((a, b) => new Date(a.start.replace(' ', 'T')) - new Date(b.start.replace(' ', 'T')));
    ...
    allRF.forEach(item => {
        ...
        const checkChange = (itm) => {
            ...
            const timeDiff = Math.abs(new Date(itm.start.replace(' ', 'T')) - new Date(itm.originalStart.replace(' ', 'T'))) / 60000;
        ...
        const formatTime = (isoOrSpace) => {
            const d = new Date(isoOrSpace.replace(' ', 'T'));
        ...
        if (arrItem && depItem) {
            ...
            const nextFlt = allRF.find(i => i.reg === arrItem.reg && new Date(i.start.replace(' ', 'T')) > new Date(arrItem.start.replace(' ', 'T')));
    ...
    if (currentMsgItems.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" style="padding:20px; color:#6c757d;">해당 일자에 15분 이상 시간이 변동되거나 주기장이 변경된 스케줄이 없습니다.</td></tr>`;
    }
}
...
export function initMessageTab() {
    ...
    const targetInput = document.getElementById('msgTargetDate');
    if (targetInput) targetInput.addEventListener('change', renderMessageTable);

    const clearBtn = document.getElementById('btnMsgFilterClear');

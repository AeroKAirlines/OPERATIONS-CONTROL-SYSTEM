...
export function showRegDetail(item) {
    ...
    if (containerRegDetail && containerRegDetail.style.display === 'none') {
        containerRegDetail.style.display = 'block';
        if (iconRegDetail) iconRegDetail.textContent = '▼ 체류 상세 정보 패널 (클릭하여 닫기)';
    }

    // 빈 상태 오버레이 제거
    const emptyOverlay = document.getElementById('regDetailEmpty');
    const contentArea = document.getElementById('regDetailContent');
...
export function initTimeline(groupsData, itemsData, onOpenEdit, onCloseEdit) {
    ...
    timelineReg.on('select', function (properties) {
        if (properties.items.length > 0) {
            ...
            // 🌟 [수정된 부분] 선택 해제 시: 내용 숨기고 오버레이를 다시 띄움
            const emptyOverlay = document.getElementById('regDetailEmpty');
            const contentArea = document.getElementById('regDetailContent');
